...
`Hood3Token.sol` is a minimal fixed-fee ERC-20 for an EVM launch.
...
- Decimals: `18`
- Creator fee: `2%`
- Fee receiver: the public Hood3 mechanism wallet
...
```text
Hood3Token(uint256 initialSupplyWholeTokens, address initialSupplyReceiver, address feeReceiver)
```
...
feeReceiver = <public Hood3 mechanism wallet>
```

The contract only routes the 2% fee to `feeReceiver`. It does not automatically swap tokens, enter HOOD perps, or burn tokens. Those actions should remain in the audited server-side automation/operations layer and be published as terminal receipts.
