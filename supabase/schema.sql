...
values
  ...
  ('eth_fee_policy', '{"network": "robinhood_eth", "gas_buffer_eth": 0.005, "send_rule": "route_creator_fees_to_lighter_hood_execution"}'::jsonb),
...
insert into public.longcat_config (config_key, config_value)
values ('eth_fee_policy', '{"network": "robinhood_eth", "gas_buffer_eth": 0.005, "send_rule": "route_creator_fees_to_lighter_hood_execution"}'::jsonb)
...
alter table public.longcat_wallets
  add column if not exists lighter_wallet_address text,
  add column if not exists lighter_account text;
...
begin
  if exists (
    ...
    set lighter_wallet_address = coalesce(lighter_wallet_address, hyperliquid_wallet_address);
  end if;
  ...
  if exists (
    ...
    update public.longcat_wallets
    set lighter_account = coalesce(lighter_account, hyperliquid_perp_account);
  end if;
end;
$$;
...
create table if not exists public.longcat_positions (
  id bigint generated by default as identity primary key,
  run_id uuid references public.longcat_automation_runs (id) on delete set null,
  lighter_account text not null,
  market text not null default 'HOOD',
...
begin
  if exists (
    ...
    set lighter_account = coalesce(lighter_account, hyperliquid_account);
  end if;
end;
$$;
...
comment on table public.longcat_terminal_events is
  'Append-only public receipt feed for Hood3 creator fee receipts, ETH routes, swaps, Lighter deposits, HOOD orders, and burns.';
...
comment on view public.longcat_latest_position is
  'Browser-safe latest market position readout for publishing the HOOD long backing the native leverage loop.';
